# Farasa — Complete Technical Specification v2

## Document Purpose

This is the definitive, locked specification for the Farasa project. Every architectural decision, feature, engineering principle, file path, schema, implementation detail, and design decision is documented here. This document serves as the single source of truth for development and as interview preparation material.
...
| Protocol Version  | A2UI v0.8 for agent-generated UIs                    |
...
| Token                 | Value                       | Usage                                              |
| --------------------- | --------------------------- | -------------------------------------------------- |
| `--bg-root`           | `#1c1c1e`                   | Page background, deepest layer (Apple HIG neutral) |
| `--bg-surface`        | `#2c2c2e`                   | Cards, sidebar, elevated panels                    |
| `--bg-surface-hover`  | `#323234`                   | Hovered cards, active list items                   |
| `--bg-surface-active` | `#3a3a3c`                   | Pressed states, selected items                     |
| `--bg-glass`          | `rgba(44, 44, 46, 0.72)`    | Glassmorphic surfaces (sidebar, input, dropdowns)  |
| `--bg-input`          | `rgba(44, 44, 46, 0.5)`     | Input fields, text areas                           |
| `--text-primary`      | `rgba(255, 255, 255, 0.92)` | Headings, primary content                          |
| `--text-secondary`    | `rgba(255, 255, 255, 0.55)` | Body text, descriptions                            |
| `--text-muted`        | `rgba(255, 255, 255, 0.3)`  | Timestamps, metadata, placeholders                 |
| `--text-ghost`        | `rgba(255, 255, 255, 0.14)` | Disabled text, very subtle labels                  |
| `--border-subtle`     | `rgba(255, 255, 255, 0.09)` | Subtle separation lines                            |
| `--border-default`    | `rgba(255, 255, 255, 0.16)` | Default borders on inputs, cards                   |
| `--accent`            | `#22d3ee`                   | Primary interactive elements, links, focus rings   |
| `--accent-hover`      | `#4ae4f8`                   | Hovered interactive elements                       |
| `--accent-muted`      | `rgba(34, 211, 238, 0.12)`  | Accent backgrounds (badges, highlights)            |
| `--accent-glow`       | `rgba(34, 211, 238, 0.22)`  | Glow effects around focused/active elements        |
| `--success`           | `#34d399`                   | Completed states, confirmations                    |
| `--warning`           | `#fbbf24`                   | Warnings, caution states                           |
| `--error`             | `#f87171`                   | Error states, destructive actions                  |
| `--thinking`          | `#a78bfa`                   | Thinking/reasoning phase accent                    |
| `--thinking-bg`       | `rgba(167, 139, 250, 0.1)`  | Thinking block background                          |
| `--thinking-border`   | `rgba(167, 139, 250, 0.16)` | Thinking block border                              |

**Provider Colors** (model badges and selector):

| Provider  | Color     | Token                  |
| --------- | --------- | ---------------------- |
| OpenAI    | `#34d399` | `--provider-openai`    |
| Anthropic | `#fb923c` | `--provider-anthropic` |
| Google    | `#60a5fa` | `--provider-google`    |
| Meta      | `#818cf8` | `--provider-meta`      |
| Groq      | `#fb7185` | `--provider-groq`      |
| Cerebras  | `#fbbf24` | `--provider-cerebras`  |
| Qwen      | `#c084fc` | `--provider-qwen`      |
| Mistral   | `#fb7185` | `--provider-mistralai` |
| DeepSeek  | `#38bdf8` | `--provider-deepseek`  |

**Light Mode**: Complete inversion with warm white backgrounds (`--bg-root: #eff1f3`, `--bg-surface: #ffffff`), dark text (`--text-primary: #0f172a`, `--text-secondary: #334155`), same accent/provider colors at adjusted saturation. Glassmorphism uses `rgba(255, 255, 255, 0.7)` with backdrop-blur.
...
| `slideInRight` | `x: 100%->0, duration: 0.35, ease: [0.4,0,0.2,1]`                   | A2UI panel                |
| `slideInLeft`  | `x: -100%->0, duration: 0.35`                                       | Sidebar opening           |
| `scaleIn`      | `scale: 0.95->1, opacity: 0->1, duration: 0.16`                     | Button press, badges      |
...
```
StreamChunk types:
  conversation_created — { conversationId }                                     Navigate to new conversation URL
  user_message_saved   — { messageId }                                          Invalidate message cache
  ...
  model_selected       — { model, reasoning, source, category, responseFormat, confidence, factors }  Auto-router / explicit result
  ...
  error                — { message, code?, reasonCode, recoverable }            Error during any phase
  done                 — { usage, terminalReason? }                             Stream complete
```

**Server-Side Emission Order**:

1. If new conversation: `conversation_created` with `conversationId`
2. After message persisted: `user_message_saved` with `messageId`
3. Model resolution: `status` with phase `routing` (if auto) or immediate `model_selected`
4. After resolution: `model_selected` with model, source, confidence, factors
5. If attachments: `status` with phase `reading_files`
6. If `webSearchEnabled`: `tool_start` (web_search) -> process -> `tool_result` with results
7. AI starts: `status` with phase `thinking`
8. If extended thinking: `thinking` chunks token-by-token
9. Text generation: `text` chunks token-by-token
10. If A2UI content: `status` generating_ui -> `a2ui` chunks
11. End: `thinking` isComplete:true (if applicable) -> `done` with usage
...
Zod-validated. Required: AUTH_SECRET, AUTH_GOOGLE_ID, AUTH_GOOGLE_SECRET, OPENROUTER_API_KEY, DATABASE_URL, TAVILY_API_KEY, GCS_BUCKET_NAME, GCS_PROJECT_ID, NEXT_PUBLIC_APP_URL, NODE_ENV. Optional: AUTH_URL, GOOGLE_APPLICATION_CREDENTIALS, RUNTIME_CONFIG_JSON, RUNTIME_CONFIG_CACHE_TTL_MS.
...
STREAM_EVENTS: CONVERSATION_CREATED, USER_MESSAGE_SAVED, STATUS, THINKING, MODEL_SELECTED, TOOL_START, TOOL_RESULT, TEXT, A2UI, ERROR, DONE.

STREAM_PHASES: ROUTING, THINKING, SEARCHING, READING_FILES, GENERATING_UI, GENERATING_TITLE.

PROVIDERS: OPENAI, ANTHROPIC, GOOGLE, META, GROQ, CEREBRAS, QWEN, MISTRALAI, DEEPSEEK.

MODEL_IDS: LLAMA_31_8B, GEMINI_FLASH_LITE, GEMINI_3_FLASH_PREVIEW. Auto-router model via `runtimeConfig.models.autoRouterModel`.
...
MOTION: DURATION_FAST (0.16), DURATION_NORMAL (0.2), DURATION_MEDIUM (0.25), DURATION_SLOW (0.35), STAGGER_CHILDREN (0.05), SPRING_STIFFNESS (400), SPRING_DAMPING (25), EASING [0.4, 0, 0.2, 1].
...
MessageRoleSchema, AttachmentSchema, UsageSchema, StreamPhaseSchema, StreamChunkSchema (full discriminated union with conversation_created/user_message_saved/status/thinking/model_selected/tool_start/tool_result/text/a2ui/error/done), ChatInputSchema (with mode: chat|team), MessageMetadataSchema (with thinkingContent, thinkingDurationMs, toolCalls, routerReasoning, `teamId?: uuid`, `isTeamSynthesis?: boolean`, `userMessageId?: uuid`, etc.). All types derived via z.infer.
...
UserPreferencesSchema, UpdateUserPreferencesSchema. Derived types.

### runtime-config.ts

RuntimeConfigSchema (full runtime config with models, prompts, limits, safety policies), RuntimeConfigOverrideSchema, RuntimeA2UIPolicy type, InvalidateRuntimeConfigInputSchema. Derived types.
...
│   ├── (protected)/chat/[[...id]]/page.tsx, layout.tsx, page.tsx
│   ├── api/trpc/[trpc]/route.ts, auth/[...nextauth]/route.ts, health/route.ts, voice/synthesize/route.ts
│   ├── offline/page.tsx
│   ├── layout.tsx, manifest.ts, globals.css, sw.ts
├── server/
│   ├── routers/_app.ts, chat.ts, conversation.ts, team.ts, model.ts, search.ts, upload.ts, runtime-config.ts, user-preferences.ts
│   ├── services/a2ui-message-service.ts, history-builder.ts, model-resolution-service.ts, search-enrichment-service.ts, search-tool-service.ts
│   ├── trpc.ts, context.ts
├── schemas/
│   ├── message.ts, conversation.ts, team.ts, model.ts, search.ts, upload.ts, auth.ts, runtime-config.ts, user-preferences.ts, index.ts
├── features/
│   ├── chat/
...
│   │   └── types.ts
│   ├── stream-phases/
│   │   ├── components/stream-progress, thinking-block, model-badge, tool-execution, routing-decision-block
...
│   ├── pwa/components/dev-sw-reset  # PWA service worker dev utilities
│   ├── voice/components/mic-button, tts-controls; hooks/use-speech-to-text, use-text-to-speech
├── lib/
│   ├── ai/client.ts, router.ts, registry.ts, tools.ts, title.ts
│   ├── search/tavily.ts
│   ├── db/schema.ts, relations.ts, client.ts, migrate.ts, seed.ts, utils.ts
│   ├── upload/gcs.ts
│   ├── auth/config.ts, edge-config.ts, shared-config.ts
│   ├── security/rate-limit.ts, token-crypto.ts, runtime-safety.ts
│   ├── runtime-config/service.ts
│   ├── utils/cn.ts, format.ts, errors.ts, motion.ts
├── config/constants.ts, routes.ts, env.ts, prompts.ts
├── components/error-boundary.tsx, session-provider.tsx, theme-provider.tsx, ui/ (shadcn)
