...
2. surfaceUpdate — defines the component tree as a FLAT array. Each component has a unique "id" and a "component" object keyed by type name. Layout components reference children by ID using "children":{"explicitList":[...]}:
   {"surfaceUpdate":{"surfaceId":"surface_main","components":[{"id":"root","component":{"Column":{"children":{"explicitList":["card1"]}}}},{"id":"card1","component":{"Card":{"child":"card_body"}}},{"id":"card_body","component":{"Column":{"children":{"explicitList":["title","name_field","meal_choice","btn_row"]}}}},{"id":"title","component":{"Text":{"text":{"literalString":"Contact Form"},"usageHint":"h2"}}},{"id":"name_field","component":{"TextField":{"label":{"literalString":"Full Name"},"text":{"path":"name_field"}}}},{"id":"meal_choice","component":{"MultipleChoice":{"label":{"literalString":"Meal Preference"},"selections":{"path":"meal_choice"},"options":[{"value":"vegetarian","label":{"literalString":"Vegetarian"}},{"value":"meat","label":{"literalString":"Meat"}}]}}},{"id":"btn_row","component":{"Row":{"children":{"explicitList":["submit_btn","cancel_btn"]}}}},{"id":"submit_btn","component":{"Button":{"child":"submit_label","primary":true,"action":{"name":"submit_form","context":[{"key":"name","value":{"path":"name_field"}},{"key":"meal","value":{"path":"meal_choice"}}]}}}},{"id":"submit_label","component":{"Text":{"text":{"literalString":"Submit"}}}},{"id":"cancel_btn","component":{"Button":{"child":"cancel_label","action":{"name":"cancel_form"}}}},{"id":"cancel_label","component":{"Text":{"text":{"literalString":"Cancel"}}}}]}}

Supported component types (use ONLY these exact names):
${A2UI_CATEGORIZED_LIST}

Value sources: Two types exist — {"literalString":"..."} for read-only display text (Text labels, headings) and {"path":"component_id"} for writable form data (TextField text, MultipleChoice selections, CheckBox/Slider/DateTimeInput value). The path should match the component's id so user input is stored in the data model.
Button uses "child" to reference a Text component by ID for its label — always create a separate Text component for each button's label text. Card uses "child" similarly.
Button actions use {"action":{"name":"action_name","context":[{"key":"field_name","value":{"path":"component_id"}}]}} — the action value MUST be an object with a "name" key, NOT a raw string. Include "context" entries referencing form field paths so the action carries submitted data.
Valid usageHint values for Text: h1, h2, h3, h4, h5, caption, body.
