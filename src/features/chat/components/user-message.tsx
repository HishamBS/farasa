...
export function UserMessage({ content, attachments }: UserMessageProps) {
  ...
  return (
    <motion.div className="flex justify-end" {...(shouldReduce ? {} : fadeInUp)}>
      <div className="max-w-[72%] overflow-hidden rounded-2xl rounded-br-sm border border-[--border-user-message] bg-[--bg-user-message] shadow-[0_2px_8px_rgba(0,0,0,0.15)]">
        ...
        <p className="whitespace-pre-wrap px-3.5 py-2.5 text-sm leading-relaxed text-[--text-primary]">
          {content}
        </p>
      </div>
      <AnimatePresence>
