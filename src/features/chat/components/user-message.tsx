...
export function UserMessage({ content, attachments }: UserMessageProps) {
  ...
  return (
    <motion.div className="flex justify-end" {...(shouldReduce ? {} : fadeInUp)}>
      <div className="max-w-[72%] overflow-hidden rounded-[18px] rounded-br-[4px] bg-[--bg-user-message] border border-[--border-user-message] shadow-[0_2px_8px_rgba(0,0,0,0.15)]">
        ...
        <p className="px-3.5 py-2.5 text-sm leading-[1.55] text-[--text-primary] whitespace-pre-wrap">
          {content}
        </p>
      </div>
      <AnimatePresence>
